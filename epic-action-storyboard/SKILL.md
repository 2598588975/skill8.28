...
description: Convert action, superpower, fantasy, disaster, large-scale conflict, VFX-heavy, or one-take scenes into cinematic text storyboards or copy-ready AI-video prompts with explicit first frames, action-camera causality, physical VFX feedback, sound, dialogue, and continuity. Use when the user asks for 史诗感动作分镜、大片感、电影级特效分镜、超能力/魔法/火焰/能量体镜头、重量级打斗、CG渲染规格、一镜到底、摇臂长镜头, or epic action shot prompts. Use fight-line-storyboard instead when the requested deliverable is a tactical line-art fight board with attack-defense blocking and motion diagrams.
---
...
When the deliverable is a single prompt intended to be pasted directly into an AI-video model, also read [references/model-facing-action-prompt.md](references/model-facing-action-prompt.md). Its first-frame-priority and compact-output rules override the legacy production-board output template and default numeric camera metadata, but do not override the source story, timing, assets, dialogue, physics, or continuity.
...
- When the user asks for a prompt that can be copied directly into Seedance, Higgsfield, Kling, Runway, Veo, or a similar generator, use the compact model-facing mode instead of automatically expanding every shot into a technical production-board form.
...
- Treat the first visible frame of every shot as the highest-priority composition lock. State which subjects are already visible, whether the face is frontal, three-quarter, profile, or back-facing, which eyes/hands/props are visible, where they sit on screen, and what action state already exists.
- Prefer visible-result camera language such as “正面三分之四近景，脸和右手同时可见” or “贴近水面的侧面高速跟拍”. Do not expect labels such as “侧前方” alone, or exact distances and angles, to resolve front/back ambiguity.
- Numeric focal lengths, fields of view, camera distances, roll angles, and camera heights are optional controls. Omit them by default in model-facing prompts; include them only when the user supplied them, a specific scale transition depends on them, or a demonstrated generation failure requires them.
...
For a direct-to-model prompt, follow the compact model-facing reference and preserve any concise format the user has already shown to work. Do not add the production-board sections below unless the user requests a technical storyboard, CG handoff, lighting sheet, or per-shot English prompts.

For a production storyboard or CG handoff, start with:
